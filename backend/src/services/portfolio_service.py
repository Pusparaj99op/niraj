...
from ..models.portfolio import (
    Portfolio, PortfolioORM, PortfolioCreateRequest, PortfolioUpdateRequest,
    PortfolioResponse, PortfolioQueryRequest, PortfolioAggregateResponse,
    PortfolioNotFoundError, PortfolioValidationError, PortfolioCalculationError,
    PositionType, PositionStatus, RiskLevel, calculate_portfolio_summary
)
...
class PortfolioService:
        ...
        try:
            ...
            async with self.db_manager.get_session() as session:
                ...
                if not include_inactive:
                    query = query.where(PortfolioORM.position_status == PositionStatus.ACTIVE.value)
            ...
            raise PortfolioServiceError(
                f"Failed to retrieve portfolio: {str(e)}",
    ...
    async def get_portfolio_position(
        ...
        try:
            ...
            cached_result = await self.cache_manager.get(cache_key)
            if cached_result:
                logger.debug("Portfolio position retrieved from cache", portfolio_id=portfolio_id)
                return cached_result

            async with self.db_manager.get_session() as session:
                query = select(PortfolioORM).where(PortfolioORM.portfolio_id == portfolio_id)
                result = await session.execute(query)
        ...
        try:
            ...
            async with self.db_manager.get_session() as session:
                ...
                for position in updated_positions:
                    ...
                    portfolio_response = await self.get_portfolio_position(position['portfolio_id'])
    ...
    async def get_portfolio_summary(
        ...
        try:
            # Check cache first
            cache_key = f"{self._portfolio_summary_cache_prefix}{user_id}:{include_inactive}"
            cached_result = await self.cache_manager.get(cache_key)
            if cached_result:
            ...
            # Get full portfolio
            positions, summary = await self.get_user_portfolio(user_id, include_inactive)
    ...
    async def validate_portfolio_risks(
        ...
        try:
            ...
            logger.error(
                "Failed to validate portfolio risks",
                user_id=user_id,
                error=str(e)
            )
            raise PortfolioServiceError(
    ...
    def _orm_to_portfolio(self, orm: PortfolioORM) -> Portfolio:
        ...
        return Portfolio(
            ...
            stop_loss_level=Decimal(str(orm.stop_loss_level)) if orm.stop_loss_level else None,
            take_profit_level=Decimal(str(orm.take_profit_level)) if orm.take_profit_level else None,
    ...
    def _portfolio_to_orm(self, portfolio: Portfolio) -> PortfolioORM:
        ...
        return PortfolioORM(
            ...
            stop_loss_level=float(portfolio.stop_loss_level) if portfolio.stop_loss_level else None,
            take_profit_level=float(portfolio.take_profit_level) if portfolio.take_profit_level else None,
