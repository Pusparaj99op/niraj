...
class AuditBatchProcessor:
    ...
    async def _flush_batch(self) -> bool:
        ...
        try:
            ...
            logger.error(
                "Batch flush failed", error=str(e), batch_size=len(batch_to_process)
            )
...
class AuditService:
    ...
    - Secure audit logging with integrity checks - Batch processing for performance - Automatic data retention management - Advanced search and filtering capabilities - Real-time monitoring and alerts -
    ...
    def __init__(
        ...
        self.performance_monitor = (
            AuditPerformanceMonitor() if enable_performance_monitoring else None
        )

        # Batch processing
        self.batch_processor = (
            AuditBatchProcessor(batch_size) if enable_batch_processing else None
        )
        ...
        **kwargs,
        ...
        try:
            ...
            raise AuditServiceError(
                f"Audit log creation failed: {str(e)}", event_type.value
            )

    async def _write_audit_log_direct(
        self, audit_log: AuditLog, start_time: float
    ) -> AuditLogResponse:
        ...
        try:
            ...
            logger.error(
                "Audit log integrity error", audit_id=audit_log.id, error=str(e)
            )
            raise AuditServiceError(
                f"Data integrity violation: {str(e)}", audit_log.event_type.value
            )

        except SQLAlchemyError as e:
            logger.error(
                "Database error during audit log creation",
                audit_id=audit_log.id,
                error=str(e),
            )
            raise AuditServiceError(
                f"Database error: {str(e)}", audit_log.event_type.value
            )
    ...
    async def create_audit_logs_batch(
        ...
        try:
            ...
            async with self.db_manager.get_transaction() as session:
                ...
                logger.info(
                    "Batch audit log creation completed",
        ...
        try:
            async with self.db_manager.get_async_session() as session:
                ...
                if query_request.event_type:
                    filters.append(
                        AuditLogORM.event_type == query_request.event_type.value
                    )
                ...
                if query_request.correlation_id:
                    filters.append(
                        AuditLogORM.correlation_id == query_request.correlation_id
                    )

                if query_request.timestamp_after:
                    filters.append(
                        AuditLogORM.timestamp >= query_request.timestamp_after
                    )

                if query_request.timestamp_before:
                    filters.append(
                        AuditLogORM.timestamp <= query_request.timestamp_before
                    )
                ...
                if query_request.tags:
                    for tag in query_request.tags:
                        filters.append(AuditLogORM.tags.contains(json.dumps([tag])))
    ...
    async def get_audit_log_by_id(
        self, audit_id: str, user_has_sensitive_access: bool = False
        ...
        try:
            ...
            logger.error(
                "Failed to get audit log by ID", audit_id=audit_id, error=str(e)
            )
    ...
    async def _detect_suspicious_patterns(
        ...
        try:
            ...
            unusual_hours_query = (
                ...
                .where(
                    or_(
                        func.strftime("%H", AuditLogORM.timestamp)
                        < "03",  # Before 9 AM IST (UTC+5:30)
                        func.strftime("%H", AuditLogORM.timestamp)
                        > "10",  # After 4 PM IST
                    )
                )
            ...
            try:
                ...
                if not self._shutdown:
                    ...
                    if cleanup_result.get("cleaned", 0) > 0:
                        logger.info(
                            "Automatic retention cleanup completed", **cleanup_result
                        )
    ...
    async def verify_audit_log_integrity(self, audit_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Integrity verification error", audit_id=audit_id, error=str(e)
            )
            ...
            async with self.db_manager.get_async_session() as session:
                ...
                for log_orm in logs:
                    try:
                        ...
                        verification_results["invalid"] += 1
                        verification_results["failed_verifications"].append(
                            {"id": log_orm.id, "error": str(e)}
                        )

                logger.info(
                    "Bulk integrity verification completed", **verification_results
                )
            ...
            try:
                ...
                try:
                    ...
                    await audit_service.create_audit_log(
                        event_type=event_type,
                        event_description=description
                        or f"{func.__name__} executed successfully",
                        severity=severity,
                ...
                logger.error(
                    "Audit decorator error", error=str(e), function=func.__name__
                )
