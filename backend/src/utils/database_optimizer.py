...
- Dynamic index recommendation and creation
...
class QueryStats:
    ...
    def update(self, execution_time_ms: float, result_count: int = 0, error: bool = False):
        ...
        self.result_count_stats[count_bucket] = self.result_count_stats.get(count_bucket, 0) + 1
...
class QueryNormalizer:
    ...
    def extract_tables(query: str) -> Set[str]:
        ...
        dml_matches = re.finditer(r'\b(?:UPDATE|INSERT\s+INTO|DELETE\s+FROM)\s+(\w+)', query.upper())
    ...
    def extract_where_columns(query: str) -> Set[str]:
        ...
        where_pattern = r'\bWHERE\s+(.+?)(?:\s+ORDER\s+BY|\s+GROUP\s+BY|\s+HAVING|\s+LIMIT|$)'
...
class QueryAnalyzer:
    ...
    def get_slow_queries(self, threshold_ms: float = 100.0, min_executions: int = 5) -> List[QueryStats]:
        """Get queries that are consistently slow"""
        with self._lock:
            slow_queries = []
            for stats in self.query_stats.values():
                if (stats.execution_count >= min_executions and
                    stats.avg_time_ms >= threshold_ms):
    ...
    def _analyze_table_indexes(self, table_name: str, queries: List[QueryStats]) -> List[IndexRecommendation]:
        ...
        for column, usage_count in where_columns.items():
            if usage_count > total_executions * 0.1:  # Used in >10% of queries
                ...
                recommendations.append(IndexRecommendation(
                    ...
                    columns=[column],
                    index_type='btree',
                    estimated_benefit=benefit * 0.7,  # Single column indexes have moderate benefit
                    reason=f"Column {column} frequently used in WHERE clauses",
                    query_patterns=[q.normalized_query for q in queries if column in QueryNormalizer.extract_where_columns(q.normalized_query)][:3]
                ))
        ...
        if len(where_columns) >= 2:
            ...
            column_pairs = []
            for query_stats in queries:
                cols = list(QueryNormalizer.extract_where_columns(query_stats.normalized_query))
                if len(cols) >= 2:
            ...
            for (col1, col2), count in pair_counts.items():
                if count > total_executions * 0.05:  # Used in >5% of queries
                    ...
                    recommendations.append(IndexRecommendation(
                        ...
                        columns=[col1, col2],
                        index_type='btree',
                        estimated_benefit=benefit * 0.9,  # Composite indexes can have high benefit
                        reason=f"Columns {col1}, {col2} frequently used together",
                        query_patterns=[q.normalized_query for q in queries
                                      if col1 in QueryNormalizer.extract_where_columns(q.normalized_query)
                                      and col2 in QueryNormalizer.extract_where_columns(q.normalized_query)][:3]
                    ))
    ...
    def get_pool_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                **self.stats,
                **pool_info,
                'efficiency': self.stats['connections_checked_in'] / max(1, self.stats['connections_checked_out']),
                'error_rate': self.stats['connection_errors'] / max(1, self.stats['connections_created'])
            }
...
class QueryCache:
    ...
    def __init__(self, cache_manager: Optional[CacheManager] = None, max_memory_mb: int = 100):
    ...
    async def set(self, query: str, result: Any, params: Optional[Dict] = None, ttl_override: Optional[int] = None):
        """Cache query result"""
        cache_key = self.get_cache_key(query, params)
        query_type = self._get_query_type(query)
        ttl = ttl_override or self._ttl_settings.get(query_type, self._ttl_settings['DEFAULT'])
    ...
    async def _store_in_memory_cache(self, cache_key: str, result: Any, query: str, ttl: int = 300):
        ...
        if len(serialized_result) > 1024:  # Compress if > 1KB
            try:
                ...
                if len(compressed_result) < len(serialized_result) * 0.8:  # At least 20% compression
        ...
        with self._lock:
            ...
            new_size = self._cache_stats['memory_usage_bytes'] + cache_item['size_bytes']
    ...
    async def _evict_items(self, needed_bytes: int):
        ...
        for cache_key, cache_item in self._memory_cache.items():
            ...
            age_hours = (datetime.now(timezone.utc) - cache_item['last_accessed']).total_seconds() / 3600
        ...
        logger.debug("Cache eviction completed",
                    ...
                    evicted_items=len([x for x in items_by_priority if x[1] not in self._memory_cache]))
    ...
    async def invalidate_pattern(self, pattern: str):
        ...
        if self.cache_manager:
            try:
                ...
                logger.warning("Error invalidating distributed cache pattern", error=str(e))

        logger.info("Cache invalidation completed", pattern=pattern, invalidated_keys=len(keys_to_remove))

    def get_cache_stats(self) -> Dict[str, Any]:
        """Get cache statistics"""
        with self._lock:
            hit_ratio = self._cache_stats['hits'] / max(1, self._cache_stats['hits'] + self._cache_stats['misses'])

            return {
                **self._cache_stats,
                'hit_ratio': hit_ratio,
                'items_count': len(self._memory_cache),
                'memory_usage_mb': self._cache_stats['memory_usage_bytes'] / 1024 / 1024,
                'memory_utilization': self._cache_stats['memory_usage_bytes'] / self.max_memory_bytes
            }
...
class DatabaseOptimizer:
    ...
    def __init__(
        ...
        logger.info("Database optimizer initialized", database_url=database_url[:50] + "...")
    ...
    async def initialize(self):
        ...
        if self.config.get('enable_maintenance', True):
            self._maintenance_task = asyncio.create_task(
                self._maintenance_loop(self.config.get('maintenance_interval', 1800))  # 30 minutes
            )
    ...
    async def _apply_index_recommendations(self):
        ...
        if applied_count > 0:
            logger.info("Automatic index creation completed", created_indexes=applied_count)
...
def get_database_optimizer() -> DatabaseOptimizer:
    ...
    if _database_optimizer is None:
        raise RuntimeError("Database optimizer not initialized. Call initialize_database_optimization first.")
