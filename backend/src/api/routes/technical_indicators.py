...
- Moving averages (SMA, EMA, WMA) - Momentum indicators (RSI, MACD, Stochastic) - Volatility indicators (Bollinger Bands, ATR) - Volume indicators (OBV, VWAP) - Custom indicator combinations -
Real-time indicator updates

Endpoints:
- GET /api/v1/indicators/{symbol}: Get all indicators for a symbol - GET /api/v1/indicators/{symbol}/{indicator}: Get specific indicator - POST /api/v1/indicators/calculate: Calculate custom indicators - GET /api/v1/indicators/screener: Screen symbols by indicator criteria -
...
class IndicatorRequest(BaseModel):
    ...
    timeframe: TimeFrame = Field(
        default=TimeFrame.FIFTEEN_MINUTES, description="Time frame"
    )
    period: int = Field(
        default=14, ge=1, le=200, description="Default period for indicators"
    )
    data_points: int = Field(
        default=100, ge=50, le=1000, description="Number of data points"
    )
...
class CustomIndicatorRequest(BaseModel):
    ...
    indicators: Dict[str, Dict[str, Any]] = Field(
        description="Indicators with parameters"
    )
    ...
    **kwargs,
    ...
    if indicator_name == "rsi":
        ...
        if len(previous_values) >= 3:
            trend = sum(
                [
                    1 if previous_values[i] < previous_values[i + 1] else -1
...
@router.get(
    ...
    **Features:** - Multiple indicator calculations - Signal generation - Overall market assessment -
    ...
    try:
        with structlog.contextvars.bound_contextvars(
            ...
            cache_key_str = cache_key(
                "all", symbol, timeframe.value, period, data_points
            )
            cached_data = await get_cached_indicators(
            ...
            if macd_data["macd"] and macd_data["macd"][-1] is not None:
                ...
                indicators["MACD"] = IndicatorData(
                    ...
                    values=[
                        IndicatorValue(timestamp=timestamps[i], value=val, signal=None)
                        for i, val in enumerate(macd_data["macd"])
                        if val is not None
                    ][-20:],
            ...
            summary = {
                ...
                "average_signal_strength": (
                    sum(ind.signal_strength for ind in indicators.values())
                    / len(indicators)
                    if indicators
                    else 0
                ),
...
@router.get(
    ...
    **Available Indicators:** - SMA, EMA, WMA (Moving Averages) - RSI (Relative Strength Index) - MACD (Moving Average Convergence Divergence) - BB (Bollinger Bands) -
        ...
        with structlog.contextvars.bound_contextvars(
            ...
            analysis = {
                "trend": (
                    ...
                    else "bearish" if indicator_data.signal == "sell" else "neutral"
                ),
                "volatility": (
                    "high"
                    if indicator_data.signal_strength > 0.7
                    else "medium" if indicator_data.signal_strength > 0.3 else "low"
                ),
                "reliability": (
                    "high" if indicator_data.signal_strength > 0.6 else "medium"
                ),
            ...
            if indicator_data.signal == "buy" and indicator_data.signal_strength > 0.6:
                recommendations.append(
                    f"Consider long position based on {indicator_data.name}"
                )
            elif (
                indicator_data.signal == "sell" and indicator_data.signal_strength > 0.6
            ):
                recommendations.append(
                    f"Consider short position based on {indicator_data.name}"
                )
            else:
                recommendations.append(
                    f"Monitor {indicator_data.name} for clearer signals"
                )

            if indicator.value == "rsi" and indicator_data.current_value > 80:
                recommendations.append(
                    "RSI shows extreme overbought conditions - exercise caution"
                )
            elif indicator.value == "rsi" and indicator_data.current_value < 20:
                recommendations.append(
                    "RSI shows extreme oversold conditions - potential reversal"
                )
