...
from ...ai.gemma3_integration import (
    Gemma3Client, AnalysisRequest, AnalysisType, AnalysisResponse
)
...
class NewsAnalysisRequest(BaseModel):
    ...
    include_social_sentiment: bool = Field(default=False, description="Include social media sentiment")
...
class MarketAnalysisRequest(BaseModel):
    ...
    sector_data: Optional[Dict[str, Any]] = Field(None, description="Sector information")
    analysis_focus: List[str] = Field(default=["trend", "momentum", "volatility"], description="Analysis focus areas")
...
class PredictionRequest(BaseModel):
    ...
    include_factors: bool = Field(default=True, description="Include influencing factors")


class SignalsRequest(BaseModel):
    """Trading signals request"""
    symbols: List[str] = Field(description="Symbols for signal generation", max_length=20)
...
async def analyze_technical(
    ...
    try:
        with structlog.contextvars.bound_contextvars(
            ...
            # Check cache
            cache_key_str = cache_key("technical", request.symbol, request.timeframe, len(request.price_data))
            cached_data = await get_cached_analysis(cache_key_str, ttl=900)  # 15 minute cache
            ...
            analysis_request = AnalysisRequest(
                ...
                input_data={
                    ...
                    "volume_data": request.volume_data[-100:] if request.volume_data else None,
                    "analysis_depth": request.analysis_depth
                },
            ...
            for indicator, value in request.indicators.items():
                indicators_analysis[indicator] = {
                    "value": value,
                    "signal": "bullish" if value > 50 else "bearish" if indicator == "RSI" else "neutral",
            ...
            support_resistance = {
                ...
                "resistance_levels": sorted(set(highs), reverse=True)[:3] if highs else []
            }
            ...
            # Cache response
            await set_cached_analysis(cache_key_str, response.dict(), ttl=900)
...
async def analyze_news_sentiment(
    ...
    try:
        with structlog.contextvars.bound_contextvars(
            ...
            cache_key_str = cache_key("news_sentiment", request.symbol or "general", articles_hash)
            cached_data = await get_cached_analysis(cache_key_str, ttl=3600)  # 1 hour cache
            ...
            for article in request.articles:
                ...
                sentiment_score = 0.0
                positive_words = ["good", "growth", "profit", "increase", "up", "bullish", "positive", "gain"]
                negative_words = ["bad", "loss", "decrease", "down", "bearish", "negative", "decline", "fall"]
            ...
            response = NewsAnalysisResponse(
                ...
                key_themes=result_data.get("key_themes", ["market", "trading", "investment"]),
                sentiment_trend=[],  # Could be implemented with historical data
                impact_assessment={
                    "short_term_impact": abs(overall_sentiment) * 0.5,
                    "market_moving_potential": 1.0 if abs(overall_sentiment) > 0.7 else 0.5,
            ...
            # Cache response
            await set_cached_analysis(cache_key_str, response.dict(), ttl=3600)
...
async def generate_trading_signals(
    ...
    try:
        with structlog.contextvars.bound_contextvars(
            ...
            # Check cache
            cache_key_str = cache_key("signals", str(request.symbols), request.timeframe, request.risk_level)
            cached_data = await get_cached_analysis(cache_key_str, ttl=300)  # 5 minute cache
            ...
            for symbol in request.symbols:
                try:
                    ...
                    base_price = 100.0  # In real implementation, get current market price
                    ...
                    signal = TradingSignal(
                        ...
                        entry_price=entry_price if signal_type != SignalType.HOLD else None,
                    ...
                    logger.warning("Failed to generate signal", symbol=symbol, error=str(e))
                    continue
            ...
            # Cache response
            await set_cached_analysis(cache_key_str, response.dict(), ttl=300)
...
async def get_ai_models(
    ...
    try:
        with structlog.contextvars.bound_contextvars(
            ...
            logger.info("AI models information retrieved", total=len(models), active=active_models)
            return response
...
async def ai_health_check(
    ...
    try:
        ...
        try:
            ...
            health_status["services"]["ai_client"] = {"status": "unavailable", "error": str(e)}
            health_status["status"] = "degraded"

        # Check cache
        if _cache_manager:
            try:
                test_key = "ai:health_test"
                await _cache_manager.set(test_key, {"test": True}, ttl=10)
                await _cache_manager.get(test_key)
