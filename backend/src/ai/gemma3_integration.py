...
class Gemma3Client:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Successfully connected to Ollama server", model=self.model_name
            )
            return True
    ...
    async def _initialize_model_tracking(self):
        ...
        try:
            ...
            logger.info(
                "AI model tracking initialized", model_id=self.ai_model.model_id
            )
    ...
    async def analyze(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        try:
            ...
            confidence_score = self._calculate_confidence(
                result, request.confidence_threshold
            )
    ...
    def _build_prompt(self, request: AnalysisRequest) -> str:
        ...
        if request.analysis_type == AnalysisType.MARKET_SENTIMENT:
            ...
            prompt = base_prompt + self._build_price_prediction_prompt(
                request.input_data
            )
        elif request.analysis_type == AnalysisType.TECHNICAL_ANALYSIS:
            prompt = base_prompt + self._build_technical_analysis_prompt(
                request.input_data
            )
            ...
            prompt = base_prompt + self._build_risk_assessment_prompt(
                request.input_data
            )
        elif request.analysis_type == AnalysisType.STRATEGY_RECOMMENDATION:
            prompt = base_prompt + self._build_strategy_recommendation_prompt(
                request.input_data
            )
        elif request.analysis_type == AnalysisType.PATTERN_RECOGNITION:
            prompt = base_prompt + self._build_pattern_recognition_prompt(
                request.input_data
            )
        elif request.analysis_type == AnalysisType.CORRELATION_ANALYSIS:
            prompt = base_prompt + self._build_correlation_analysis_prompt(
                request.input_data
            )
        else:
            prompt = (
                base_prompt
                + f"Analyze the following data: {json.dumps(request.input_data)}"
            )
    ...
    def _build_sentiment_prompt(self, data: Dict[str, Any]) -> str:
        ...
        - News headlines and content - Social media sentiment - Market indicators -
    ...
    def _build_price_prediction_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Consider:
        - Historical price patterns - Technical indicators - Volume analysis - Market trends -
    ...
    def _build_technical_analysis_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Analyze:
        - Chart patterns - Technical indicators (RSI, MACD, Moving averages) - Support and resistance levels - Volume patterns -
        Trend analysis
    ...
    def _build_news_analysis_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Evaluate:
        - News sentiment and tone - Potential market impact - Relevance to specific stocks/sectors -
    ...
    def _build_risk_assessment_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Evaluate:
        - Market risk factors - Volatility levels - Position sizing appropriateness - Correlation risks -
        Liquidity concerns
    ...
    def _build_strategy_recommendation_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Consider:
        - Market regime (bull/bear/sideways) - Volatility environment - Available instruments - Risk tolerance -
        Time horizon
    ...
    def _build_pattern_recognition_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Look for:
        - Chart patterns (triangles, flags, head & shoulders) - Candlestick patterns - Volume patterns - Breakout/breakdown patterns -
        Seasonal patterns
    ...
    def _build_correlation_analysis_prompt(self, data: Dict[str, Any]) -> str:
        ...
        Examine:
        - Asset correlations - Sector correlations - Market factor correlations -
        ...
        try:
            ...
            for required_field in required_fields:
                if required_field not in result:
                    result[required_field] = "Not provided"
            ...
            logger.warning(
                "Failed to parse JSON response", raw_response=raw_response[:200]
            )
            return {
                "analysis": raw_response,
                "confidence": 0.3,
    ...
    def _update_performance_metrics(self, processing_time: float):
        ...
        if self.ai_model:
            self.ai_model.record_prediction(
                success=True, inference_time_ms=processing_time
            )
...
async def predict_price(
    symbol: str, market_context: MarketContext, timeframe: str = "1d"
) -> AnalysisResponse:
    """Get price prediction"""
    async with gemma3_client:
        return await gemma3_client.predict_price_movement(
            symbol, market_context, timeframe
        )


async def get_strategy_recommendation(
    market_regime: str, risk_tolerance: str, capital: float
) -> AnalysisResponse:
    """Get strategy recommendation"""
    async with gemma3_client:
        return await gemma3_client.recommend_strategies(
            market_regime, risk_tolerance, capital
        )
...
async def example_usage():
    ...
    async with gemma3_client:
        ...
        price_prediction = await gemma3_client.predict_price_movement(
            symbol="BANKNIFTY", market_context=market_context, timeframe="1h"
        )
        ...
        strategy_rec = await gemma3_client.recommend_strategies(
            market_regime="bullish", risk_tolerance="medium", available_capital=100000.0
        )
