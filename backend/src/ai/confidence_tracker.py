...
Features:
- Dynamic confidence calibration based on historical performance
- Multi-dimensional confidence scoring (accuracy, timing, stability)
- Learning mechanisms for confidence improvement
- Comprehensive error handling and recovery
- Integration with AI models and predictions
...
class ModelConfidenceProfile:
    ...
    type_confidence: Dict[PredictionType, ConfidenceMetrics] = field(default_factory=dict)
...
class AdvancedConfidenceTracker:
    ...
    def __init__(self):
        ...
        # Calibration data
        self.calibration_data: Dict[str, Dict[str, List]] = defaultdict(lambda: defaultdict(list))
    ...
    async def track_prediction_confidence(
        ...
        try:
            ...
            await self._handle_tracking_error(e, prediction.model_id if prediction else "unknown")
            raise ConfidenceTrackingError(f"Failed to track prediction confidence: {str(e)}")
    ...
    async def _calculate_raw_confidence_metrics(
        ...
        try:
            ...
            timing_confidence = await self._calculate_timing_confidence(prediction, market_context)
            stability_confidence = await self._calculate_stability_confidence(prediction)
            ...
            raise MetricsCalculationError(f"Failed to calculate raw confidence metrics: {str(e)}")
    ...
    async def _calculate_accuracy_confidence(self, prediction: AIPrediction) -> float:
        ...
        try:
            ...
            if prediction.prediction_type in model_profile.type_confidence:
                ...
                accuracy_confidence = (accuracy_confidence + type_metrics.accuracy_rate) / 2

            # Recent performance adjustment
            if len(model_profile.recent_performance) > 5:
                recent_accuracy = mean(model_profile.recent_performance)
                trend_factor = min(1.5, max(0.5, recent_accuracy / model_profile.overall_accuracy))
    ...
    async def _calculate_timing_confidence(
        ...
        try:
            ...
            if prediction.expiry_time:
                time_to_expiry = (prediction.expiry_time - datetime.utcnow()).total_seconds()
    ...
    async def _calculate_stability_confidence(self, prediction: AIPrediction) -> float:
        ...
        try:
            ...
            similar_predictions = [
                p for p in list(self.prediction_history)[-50:]  # Last 50 predictions
                if (p.get('model_id') == prediction.model_id and
                    p.get('prediction_type') == prediction.prediction_type.value)
            ]

            if len(similar_predictions) >= 3:
                # Calculate consistency of recent predictions
                recent_confidences = [p.get('confidence_score', 0.5) for p in similar_predictions[-5:]]
            ...
            if hasattr(prediction, 'input_features') and prediction.input_features:
                ...
                numeric_features = {
                    k: v for k, v in prediction.input_features.items()
    ...
    async def _calculate_market_confidence(self, market_context: Optional[Dict[str, Any]] = None) -> float:
        ...
        try:
            ...
            if sentiment_strength > 0.7:
                ...
                market_confidence *= 0.95  # Weak sentiment slightly decreases confidence

            # Economic indicators
            economic_indicators = market_context.get('economic_indicators', {})
            if economic_indicators:
                indicator_count = len(economic_indicators)
                positive_indicators = sum(1 for v in economic_indicators.values() if v > 0)
                indicator_ratio = positive_indicators / indicator_count if indicator_count > 0 else 0.5
    ...
    async def _calculate_adaptive_weights(self, prediction_type: PredictionType) -> Dict[str, float]:
        ...
        try:
            ...
            if prediction_type in [PredictionType.PRICE_DIRECTION, PredictionType.PRICE_TARGET]:
                base_weights['accuracy'] += 0.1
                base_weights['market'] += 0.05
                base_weights['base'] -= 0.15

            elif prediction_type in [PredictionType.BUY_SIGNAL, PredictionType.SELL_SIGNAL]:
            ...
            return {k: 0.2 for k in ['base', 'accuracy', 'timing', 'stability', 'market']}
    ...
    async def _calculate_reliability_score(
        ...
        try:
            ...
            if model_profile and model_profile.prediction_count > 10:
                ...
                reliability_from_consistency = (reliability_from_consistency + historical_reliability) / 2

            # Adjust based on prediction confidence level
            if prediction.confidence_score > 0.8:
                reliability_from_consistency *= 1.1  # High confidence predictions should be more reliable
            elif prediction.confidence_score < 0.4:
                reliability_from_consistency *= 0.9  # Low confidence predictions are less reliable
    ...
    async def _apply_confidence_calibration(
        ...
        try:
            ...
            if profile.prediction_count < self.min_samples_for_calibration:
                ...
                metrics.calibrated_confidence = metrics.raw_confidence * 0.9  # Slight conservatism
    ...
    async def _build_calibration_model(
        ...
        try:
            ...
            for record in self.prediction_history:
                if (record.get('model_id') == profile.model_id and
                    record.get('prediction_type') == prediction_type.value and
                    record.get('outcome_accuracy') is not None):
    ...
    async def _create_adaptive_bins(self, historical_data: List[Tuple[float, float]]) -> List[Dict[str, float]]:
        ...
        try:
            ...
            accuracies = [x[1] for x in historical_data]

            # Calculate optimal number of bins using Freedman-Diaconis rule
            q75, q25 = np.percentile(confidences, [75, 25])
            iqr = q75 - q25
            bin_width = 2 * iqr / (len(confidences) ** (1/3))
            num_bins = min(20, max(5, int((max(confidences) - min(confidences)) / bin_width)))
            ...
            for i in range(num_bins):
                ...
                end_idx = start_idx + bin_size if i < num_bins - 1 else len(historical_data)
                ...
                bins.append({
                    ...
                    'accuracy_std': stdev(bin_accuracies) if len(bin_accuracies) > 1 else 0.0,
            ...
            logger.warning("Failed to create adaptive bins, using simple binning", error=str(e))
            return await self._create_simple_bins(historical_data)

    async def _create_simple_bins(self, historical_data: List[Tuple[float, float]]) -> List[Dict[str, float]]:
        ...
        try:
            ...
            for i in range(self.confidence_bins):
                ...
                bin_data = [
                    (conf, acc) for conf, acc in historical_data
                    if bin_min <= conf < bin_max or (i == self.confidence_bins - 1 and conf <= bin_max)
                ]
    ...
    async def _apply_calibration_model(self, raw_confidence: float, calibration_key: str) -> float:
        ...
        try:
            ...
            for bin_data in bins:
                if bin_data['confidence_min'] <= raw_confidence <= bin_data['confidence_max']:
                    target_bin = bin_data
                    break
        ...
        try:
            ...
            if market_context:
                ...
                if volatility > 0.8:  # High volatility
                    ...
                    adjustment_factors['high_volatility_penalty'] = volatility_adjustment
    ...
    async def _get_or_create_model_profile(self, model_id: str) -> ModelConfidenceProfile:
        ...
        try:
            ...
            raise ConfidenceTrackingError(f"Failed to get/create model profile: {str(e)}")
    ...
    async def update_prediction_outcome(
        ...
        try:
            ...
            if not (0.0 <= outcome_accuracy <= 1.0):
                raise ConfidenceTrackingError("Outcome accuracy must be between 0.0 and 1.0")
            ...
            if not prediction_record:
                logger.warning("Prediction record not found for outcome update", prediction_id=prediction_id)
                return
            ...
            if model_id in self.model_profiles:
                ...
                if profile.prediction_count == 1:
                    ...
                    profile.overall_accuracy = (
                        (1 - alpha) * profile.overall_accuracy +
            ...
            raise ConfidenceTrackingError(f"Failed to update prediction outcome: {str(e)}")
    ...
    async def get_model_confidence_summary(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfidenceTrackingError(f"Failed to generate model confidence summary: {str(e)}")
    ...
    async def get_global_confidence_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_predictions = [
                record for record in list(self.prediction_history)[-1000:]
            ...
            if recent_predictions:
                total_accuracy = sum(
                    record.get('outcome_accuracy', 0.0)
    ...
    async def _calculate_global_calibration_quality(self) -> float:
        ...
        try:
            ...
            for calibration_key, cal_model in self.calibration_models.items():
                ...
                ece = 0.0
                total_samples = sum(bin_data.get('sample_count', 0) for bin_data in bins)
            ...
            logger.warning("Failed to calculate global calibration quality", error=str(e))
            return 0.0
    ...
    async def _handle_tracking_error(self, error: Exception, context: str = "") -> None:
        ...
        try:
            ...
            logger.error("Failed to handle tracking error", recovery_error=str(recovery_error))
    ...
    async def _recover_from_calibration_error(self, context: str) -> None:
        ...
        try:
            ...
            problematic_keys = [
                key for key in self.calibration_models.keys()
                if context in key
            ]

            for key in problematic_keys:
                del self.calibration_models[key]
                logger.info("Cleared problematic calibration model", calibration_key=key)
        ...
        try:
            ...
            if context in self.model_profiles:
                ...
                if profile.recovery_attempts > 5:
                    ...
                    logger.info("Reset model profile after multiple recovery attempts", model_id=context)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.calibration_models) == 0 and len(self.model_profiles) > 0:
                issues.append("No calibration models built despite having model profiles")
                health_status = "degraded"
