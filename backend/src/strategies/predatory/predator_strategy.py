...
Key Features:
- Order book analysis and depth of market monitoring - Institutional order pattern recognition - Front-running signal generation with microsecond precision - Advanced risk management for high-volatility trades - AI-powered confidence scoring and pattern recognition -
...
from src.utils.technical_indicators import (
    ...
    MarketData as TIMarketData,
)
from src.models.strategy_signal import SignalType
...
class PredatorValidator:
    ...
    def validate_market_data(market_data: Any) -> None:
        ...
        # Support both internal naming schemes: close_price vs close
        required_attrs = ["symbol", "timestamp", "volume"]
        for attr in required_attrs:
            if not hasattr(market_data, attr):
                raise ValidationError(
                    f"Market data missing required attribute: {attr}"
                )

        # Determine close price attribute name
        close_attr = "close_price" if hasattr(market_data, "close_price") else "close"
        if not hasattr(market_data, close_attr):
            raise ValidationError("Market data missing close price attribute")
        ...
        close_val = getattr(market_data, close_attr)
        if not isinstance(close_val, (int, float, Decimal)) or close_val <= 0:  # type: ignore[operator]
            raise ValidationError("Invalid close price in market data")

        volume_val = getattr(market_data, "volume")
        if not isinstance(volume_val, (int, float)) or volume_val < 0:  # type: ignore[operator]
    ...
    def validate_config(config) -> None:
        ...
        if config.strategy_type != StrategyType.PREDATORY:
            raise ConfigurationError(
                f"Invalid strategy type: {config.strategy_type}. Expected PREDATORY"
            )
        ...
        if hasattr(config, "custom_params") and config.custom_params:
            ...
            if "institutional_threshold" in custom_params:
                ...
                if not isinstance(threshold, (int, float)) or threshold <= 0:
                    raise ConfigurationError(
                        "institutional_threshold must be positive number"
                    )
            ...
            if "front_running_window" in custom_params:
                ...
                if not isinstance(window, (int, float)) or window <= 0:
                    raise ConfigurationError(
                        "front_running_window must be positive number"
                    )
    ...
    def validate_institutional_detection(detection) -> None:
        ...
        if not isinstance(detection.confidence_score, (int, float)) or not (
            0 <= detection.confidence_score <= 1
        ):
            raise InstitutionalDetectionError(
        ...
        if (
            not isinstance(detection.estimated_size, (int, float))
            or detection.estimated_size <= 0
        ):
...
class PredatorStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig, learning_engine=None):
        ...
        try:
            ...
            self.min_order_book_depth = get_config(
                "strategy.predator.min_order_book_depth", 10
            )
            if (
                not isinstance(self.min_order_book_depth, int)
                or self.min_order_book_depth < 1
            ):
                raise ConfigurationError(
                    "min_order_book_depth must be positive integer"
                )

            self.institutional_size_threshold = get_config(
                "strategy.predator.institutional_size_threshold", 1000
            )
            if (
                not isinstance(self.institutional_size_threshold, (int, float))
                or self.institutional_size_threshold <= 0
            ):
                raise ConfigurationError(
                    "institutional_size_threshold must be positive number"
                )

            self.front_running_window_seconds = get_config(
                "strategy.predator.front_running_window", 5
            )
            if (
                not isinstance(self.front_running_window_seconds, (int, float))
                or self.front_running_window_seconds <= 0
            ):
                raise ConfigurationError("front_running_window must be positive number")

            self.max_slippage_percentage = get_config(
                "strategy.predator.max_slippage", 0.1
            )
            if not isinstance(self.max_slippage_percentage, (int, float)) or not (
                0 <= self.max_slippage_percentage <= 1
            ):
            ...
            self.max_history_size = get_config(
            ...
            self.profit_taking_levels = get_config(
                "strategy.predator.profit_taking_levels", [0.5, 1.0, 2.0, 5.0, 10.0]
            )
            if not isinstance(self.profit_taking_levels, list) or not all(
                isinstance(x, (int, float)) and x > 0 for x in self.profit_taking_levels
            ):
                raise ConfigurationError(
                    "profit_taking_levels must be list of positive numbers"
                )
            ...
            logger.error(
                "Predator strategy initialization validation failed", error=str(e)
            )
            raise
        except Exception as e:
            logger.error(
                "Unexpected error during Predator strategy initialization", error=str(e)
            )
            raise ConfigurationError(
    ...
    async def _update_order_book(self, market_data: MarketData):
        ...
        try:
            ...
            for i in range(10):  # Top 10 bid levels
                ...
                quantity = int(
                    market_data.volume * (0.1 - i * 0.008)
                )  # Decreasing volume
                if quantity > 0:
                    bids.append(
                        OrderBookLevel(
    ...
    async def _calculate_technical_indicators(
        ...
        try:
            # Adapt core MarketData (with close_price etc.) into indicator calculator MarketData
            try:
                indicator_md = TIMarketData(
                    timestamp=market_data.timestamp,
                    open=Decimal(str(getattr(market_data, "open_price", getattr(market_data, "open", 0)))),
                    high=Decimal(str(getattr(market_data, "high_price", getattr(market_data, "high", 0)))),
                    low=Decimal(str(getattr(market_data, "low_price", getattr(market_data, "low", 0)))),
                    close=Decimal(str(getattr(market_data, "close_price", getattr(market_data, "close", 0)))),
                    volume=int(getattr(market_data, "volume", 0)),
                    symbol=market_data.symbol,
                )
            except Exception as e:  # pragma: no cover - defensive
                logger.warning("Failed to adapt market data for indicators", error=str(e))
                return {}

            data_points: List[TIMarketData] = [indicator_md]  # Placeholder list (historical data would extend this)
    ...
    async def _analyze_order_book_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            imbalance_analysis = {
                ...
                "strength": (
                    "strong"
                    if abs(ob.imbalance_ratio) > 0.3
                    else "moderate" if abs(ob.imbalance_ratio) > 0.1 else "weak"
                ),
            ...
            depth_analysis = {
                ...
                "spread_bps": (
                    (ob.spread / ((ob.bids[0].price + ob.asks[0].price) / 2)) * 10000
            ...
            large_orders = {
                ...
                "institutional_presence": any(
                    level.quantity >= self.institutional_size_threshold
    ...
    async def _analyze_order_clustering(
        ...
        try:
            ...
            avg_bid_per_level = (
                order_book.total_bid_volume / len(order_book.bids)
                if order_book.bids
                else 0
            )
            avg_ask_per_level = (
                order_book.total_ask_volume / len(order_book.asks)
                if order_book.asks
                else 0
            )
            ...
            iceberg_bids = [
                price
                for price, volume in bid_clustering.items()
                if volume > avg_bid_per_level * 3
            ]
            iceberg_asks = [
                price
                for price, volume in ask_clustering.items()
                if volume > avg_ask_per_level * 3
            ]
        ...
        try:
                ...
                if level.quantity >= self.institutional_size_threshold:
                    ...
                    detection = InstitutionalOrderDetection(
                        ...
                        confidence_score=min(
                            0.95,
                            level.quantity / (self.institutional_size_threshold * 2),
                        ),
            ...
            for price in clustering["iceberg_bids"] + clustering["iceberg_asks"]:
                ...
                detection = InstitutionalOrderDetection(
                    ...
                    estimated_size=clustering["bid_clustering"].get(
                        price, clustering["ask_clustering"].get(price, 0)
                    )
                    * price,
                    direction=direction,
                    detection_timestamp=current_ob.timestamp,
            ...
            if previous_ob:
                momentum_score = await self._calculate_order_book_momentum(
                    current_ob, previous_ob
                )
                if momentum_score > 0.7:
                    direction = (
                        "buy"
                        if current_ob.imbalance_ratio > previous_ob.imbalance_ratio
                        else "sell"
                    )
                    detection = InstitutionalOrderDetection(
                        ...
                        estimated_size=(
                            abs(
                                current_ob.total_bid_volume
                                - previous_ob.total_bid_volume
                            )
    ...
    async def _calculate_order_book_momentum(
        ...
        try:
            ...
            bid_volume_change = abs(
                current.total_bid_volume - previous.total_bid_volume
            )
            ask_volume_change = abs(
                current.total_ask_volume - previous.total_ask_volume
            )
            volume_change_ratio = (
                (bid_volume_change + ask_volume_change)
                / (previous.total_bid_volume + previous.total_ask_volume)
                if (previous.total_bid_volume + previous.total_ask_volume) > 0
                else 0
            )
    ...
    async def _get_ai_market_insights(
        ...
        try:
            ...
            market_context = {
                ...
                "current_price": float(market_data.close_price),
                "price_change_pct": (
                    ((market_data.close_price - market_data.open_price) / market_data.open_price) * 100
                ...
                "order_book": (
                    {
                        "spread": (
                            self.current_order_book.spread
                            if self.current_order_book
                            else 0
                        ),
                        "imbalance": (
                            self.current_order_book.imbalance_ratio
                            if self.current_order_book
                            else 0
                        ),
                        "depth": (
                            len(self.current_order_book.bids)
                            + len(self.current_order_book.asks)
                            if self.current_order_book
                            else 0
                        ),
                    }
    ...
    async def _calculate_market_microstructure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            price_range = (
                (ob.asks[-1].price - ob.bids[-1].price) if ob.asks and ob.bids else 0
            )
            avg_price = (
                (ob.bids[0].price + ob.asks[0].price) / 2 if ob.bids and ob.asks else 0
            )
    ...
    async def generate_signals(self, analysis: MarketAnalysis) -> List[TradingSignal]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Institutional detection failed, using empty list", error=str(e)
                )
            ...
            for detection in institutional_detections:
                try:
                    if detection.confidence_score >= self.min_confidence_threshold:
                        opportunity = await self._create_front_running_opportunity(
                            detection, analysis
                        )
            ...
            if len(self.active_opportunities) > self.max_active_opportunities:
                ...
                opportunities_to_remove = sorted_opportunities[
                    : len(sorted_opportunities) - self.max_active_opportunities
                ]
    ...
    async def _create_front_running_opportunity(
        self, detection: InstitutionalOrderDetection, analysis: MarketAnalysis
        ...
        try:
            ...
            size_multiplier = min(
                2.0,
                detection.estimated_size / (self.institutional_size_threshold * 1000),
            )
            position_size_percentage = (
                base_position_size * confidence_multiplier * size_multiplier
            )
            ...
            if (
                ...
                and detection.pattern_type == InstitutionalOrderPattern.LARGE_BLOCK
            ):
    ...
    async def _convert_opportunity_to_signal(
        self, opportunity: FrontRunningOpportunity, analysis: MarketAnalysis
    ) -> Optional[TradingSignal]:
        """Convert front-running opportunity to trading signal"""
        try:
            # Determine signal type
            if opportunity.signal_type in [
                FrontRunningSignal.INSTITUTIONAL_BUY,
                FrontRunningSignal.MOMENTUM_BREAKOUT,
            ]:
    ...
    async def manage_risk(self, portfolio: Any) -> List[str]:
        ...
        try:
            ...
            recent_trades = [
                trade for trade in self.executed_front_runs[-10:]
            ]  # Last 10 trades
    ...
    async def update_performance(self, trade_result: Dict[str, Any]):
        ...
        try:
            ...
            # Update win rate
            total_trades = (
                self.front_running_stats["successful_front_runs"]
                + self.front_running_stats["failed_front_runs"]
            )
            if total_trades > 0:
                self.front_running_stats["win_rate"] = (
                    self.front_running_stats["successful_front_runs"] / total_trades
                )
            ...
            if pnl < 0:
                self.front_running_stats["max_drawdown"] = max(
                    self.front_running_stats["max_drawdown"], abs(pnl)
                )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            predator_health = {
                "order_book_status": (
                    "available" if self.current_order_book else "unavailable"
                ),
                ...
                "performance_metrics": {
                    ...
                    "total_opportunities": self.front_running_stats.get(
                        "total_opportunities", 0
                    ),
                    "avg_profit_per_trade": self.front_running_stats.get(
            ...
            if self.current_order_book:
                ...
                if (
                    predator_health["order_book_age_seconds"]
                    and predator_health["order_book_age_seconds"] < 300
                ):  # 5 minutes
                    health_score += 10
            ...
            return {
                ...
                "recent_detections": len(
                    [
                        ...
                        if (datetime.utcnow() - d.detection_timestamp).seconds < 3600
                    ]
                ),  # Last hour
                "pattern_distribution": pattern_counts,
                "average_confidence": avg_confidence,
                "high_confidence_detections": len(
                    [d for d in confidence_scores if d >= 0.8]
                ),
            }
        ...
        try:
            return {
                ...
                "risk_metrics": {
                    ...
                    "avg_holding_time": self.front_running_stats[
    ...
    def _calculate_success_rate_trend(self) -> float:
        ...
        older_trades = (
            self.executed_front_runs[-20:-10]
            if len(self.executed_front_runs) >= 20
            else []
        )
        if older_trades:
            older_success_rate = sum(
                1 for trade in older_trades if trade.get("pnl", 0) > 0
            ) / len(older_trades)
    ...
    def _calculate_risk_adjusted_return(self) -> float:
        ...
        if (
            not self.executed_front_runs
            or self.front_running_stats["max_drawdown"] == 0
        ):
            return 0.0
    ...
    async def cleanup(self) -> None:
        ...
        try:
            logger.info("Cleaning up predator strategy...")
    ...
    async def optimize_resources(self):
        ...
        try:
            ...
            for opp_id, opportunity in self.active_opportunities.items():
                time_elapsed_seconds = (current_time - opportunity.creation_timestamp).total_seconds()
                if time_elapsed_seconds > opportunity.expiry_seconds:
...
async def example_predator_usage() -> None:
    """Example usage of the Predator strategy (typed & validated)."""
    ...
    try:
        ...
        if not initialized:
            ...
            return
        # Mock object with correct attribute names expected by strategy
        ...
        class MockMarketData:
            def __init__(self) -> None:
                ...
                self.high_price = 45200.0
                self.low_price = 44900.0
                self.close_price = 45150.0
                self.volume = 150000

        market_data = MockMarketData()
        analysis = await predator.analyze_market(market_data)  # type: ignore[arg-type]
    ...
    except Exception as e:  # pragma: no cover - demonstration use
        print(f"Error in predator strategy example: {str(e)}")

# Fix test at the end if it exists


async def test_predator_strategy() -> None:
    """Typed test routine for predator strategy."""
    try:
        config = StrategyConfig(
        ...
        # Construct a minimal MarketData instance using required Decimal fields
        md = MarketData(
            symbol="BTCUSD",
        ...
        analysis = await strategy.analyze_market(md)
...
async def _run_all_examples() -> None:
    """Replicate original script side-effects: example, test, example again."""
    await example_predator_usage()
    await test_predator_strategy()
    # Second run of example to mirror former duplicate main guards
    await example_predator_usage()


if __name__ == "__main__":  # pragma: no cover - execution entrypoint
    asyncio.run(_run_all_examples())
