...
class BaseStrategy(abc.ABC):
    ...
    @abc.abstractmethod
    async def calculate_position_size(self, signal: TradingSignal, portfolio: Portfolio) -> float:
        """
    ...
    async def validate_signal(self, signal: TradingSignal) -> bool:
        ...
        try:
            ...
            if signal.strength < self.config.min_signal_strength:
                logger.debug("Signal strength below threshold", signal_id=signal.signal_id)
                return False

            # Check risk parameters
            if signal.stop_loss_price and signal.entry_price:
                stop_loss_pct = abs(signal.stop_loss_price - signal.entry_price) / signal.entry_price
            ...
            trading_start = datetime.strptime(self.config.trading_hours_start, "%H:%M").time()
            trading_end = datetime.strptime(self.config.trading_hours_end, "%H:%M").time()
            ...
            logger.error("Signal validation failed", signal_id=signal.signal_id, error=str(e))
            return False

    async def execute_signal(self, signal: TradingSignal, portfolio: Portfolio) -> Optional[Trade]:
        """
        ...
        try:
            ...
            trade = Trade(
                ...
                quantity=int(position_size * portfolio.total_value / signal.entry_price),
            ...
            logger.error("Signal execution failed", signal_id=signal.signal_id, error=str(e))
            raise ExecutionError(f"Failed to execute signal: {str(e)}", self.strategy_id)
    ...
    async def manage_risk(self, portfolio: Portfolio) -> List[str]:
        ...
        try:
            ...
            for position_id, position_data in self.active_positions.items():
                ...
                if current_price:
                    ...
                    stop_loss_triggered = (trade.stop_loss and
                                           ((trade.trade_type == TradeType.BUY and current_price <= trade.stop_loss) or
                    ...
                    take_profit_triggered = (trade.take_profit and
                                             ((trade.trade_type == TradeType.BUY and current_price >= trade.take_profit) or
            ...
            for position_data in self.active_positions.values():
                ...
                exposure = position_data['trade'].quantity * position_data['trade'].price
            ...
            raise RiskManagementError(f"Risk management failed: {str(e)}", self.strategy_id)
    ...
    async def adapt_strategy(self, performance_feedback: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not self.learning_engine or not HAS_LEARNING_ENGINE or not self.config.learning_enabled:
                logger.debug("Learning not enabled for strategy", strategy_id=self.strategy_id)
                return False
            ...
            time_since_last_adaptation = (datetime.utcnow() - self.last_adaptation).total_seconds() / 60

            if (current_performance < adaptation_threshold and
                time_since_last_adaptation >= self.config.adaptation_interval_minutes):
        ...
        try:
            ...
            if self.performance.avg_loss > 0:
                self.performance.profit_factor = (
                    self.performance.avg_win * self.performance.win_rate /
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.error("Strategy initialization failed", strategy_id=self.strategy_id)
                return False
