...
class TestRAGProcessor:
    ...
    async def test_knowledge_retrieval(self):
        ...
        with patch.object(
            ...
            if result.items:
                assert (
                    "AAPL" in result.items[0].symbols
                    or "Apple" in result.items[0].content
                )
    ...
    async def test_analysis_augmentation(self):
        ...
        with patch.object(
            ...
            # Test augmentation
            enhanced_request = await self.rag_processor.augment_analysis(
                analysis_request
            )
...
class TestIntegration:
    ...
    async def test_full_rag_pipeline(self):
        ...
        with patch("src.ai.rag_processor.get_config") as mock_config:
            ...
            with patch.object(
                rag_processor, "_load_embedding_model", return_value=mock_model
            ):
                async with rag_processor:
                    ...
                    ingestion_result = await rag_processor.ingest_knowledge(
                        knowledge_items
                    )
                    ...
                    enhanced_request = await rag_processor.augment_analysis(
                        analysis_request
                    )
