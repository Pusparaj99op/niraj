...
class TestStrategiesBacktestContract:
    ...
    def test_run_backtest_success_contract(
        ...
        assert response_json["period"]["start_date"] == valid_backtest_request["start_date"]
        assert response_json["period"]["end_date"] == valid_backtest_request["end_date"]
        assert response_json["initial_capital"] == valid_backtest_request["initial_capital"]
        ...
        self._validate_performance_metrics_structure(response_json["performance_metrics"])
        ...
        assert isinstance(response_json["equity_curve"], list), "equity_curve must be array"
    ...
    def test_run_backtest_invalid_strategy_id_contract(
        ...
        response = client.post(
            f"/api/v1/strategies/{invalid_id}/backtest",
            json=valid_backtest_request
        )
    ...
    def test_run_backtest_nonexistent_strategy_contract(
        ...
        response = client.post(
            f"/api/v1/strategies/{nonexistent_id}/backtest",
            json=valid_backtest_request
        )
    ...
    def test_run_backtest_empty_symbols_array_contract(
        ...
        acceptable_codes = [200, 400, 422]  # Some systems may fall back to strategy symbols
    ...
    def test_run_backtest_large_initial_capital_contract(
        ...
        if response.status_code == 200:
            response_json = response.json()
            assert response_json["initial_capital"] == large_capital["initial_capital"], (
    ...
    def _validate_backtest_result_structure(self, result: dict) -> None:
        ...
        required_fields = [
            "strategy_id", "backtest_id", "period", "initial_capital",
            "final_capital", "performance_metrics", "trades", "equity_curve"
        ]
        ...
        assert isinstance(initial_capital, (int, float)), "initial_capital must be number"
        ...
        for curve_point in equity_curve:
            ...
            assert "portfolio_value" in curve_point, "curve point must contain portfolio_value"
            assert isinstance(curve_point["date"], str), "curve point date must be string"
            assert isinstance(curve_point["portfolio_value"], (int, float)), (
