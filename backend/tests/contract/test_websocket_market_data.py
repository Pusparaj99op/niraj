            ...
            try:
                ...
                for field in required_fields:
                    assert field in data, (
                        f"Market data must contain '{field}'"
                    )

                # Assert - Symbol and Timeframe
                assert data["symbol"] in ["BANKNIFTY"], (
                ...
                for field in ohlcv_fields:
                    assert field in ohlcv, (
                        f"OHLCV must contain '{field}'"
                    )
    ...
    async def test_websocket_market_data_multiple_symbols(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (len(received_symbols) < len(symbols) and
                    ...
                    try:
                        ...
                        if message["type"] == "market_data":
                            ...
                            assert symbol in symbols, (
                                f"Received data for unexpected symbol: "
                                f"{symbol}"
                            )

                            # Verify message format
                            assert "ohlcv" in message["data"], (
    ...
    async def test_websocket_market_data_connection_recovery(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                assert subscription_id2 != subscription_id1, (
                    "New subscription should have different ID"
                )
                ...
                if initial_timestamp and recovery_timestamp:
                    assert recovery_timestamp >= initial_timestamp, (
    ...
    async def test_websocket_market_data_concurrent_subs(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                subscription_ids = [sub["subscription_id"] for sub in
                                    active_subs]
                ...
                if subscription_ids:
                    unsubscribe_message = {
                        "type": "unsubscribe",
                        "data": {
                            "subscription_ids": [subscription_ids[0]]
                        }
                    }
