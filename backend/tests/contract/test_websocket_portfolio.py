...
class TestWebSocketPortfolioContract:
    ...
    async def test_portfolio_summary_accuracy(self):
        ...
        try:
            ...
            for update in updates:
                ...
                expected_available = (
                    summary["total_value"] - summary["margin_used"]
                )
                margin_tolerance = 0.01  # Allow small floating point errors
                margin_diff = abs(
                    summary["available_margin"] - expected_available
                )
        ...
        try:
            ...
            for update in updates:
                ...
                if "positions" in data and data["positions"]:
                    ...
                    pnl_tolerance = 0.01
                    pnl_diff = abs(
                        summary["unrealized_pnl"] - position_pnl_sum
                    )
    ...
    async def test_portfolio_margin_calculations(self):
        ...
        try:
            ...
            if update.get("type") == "PORTFOLIO_UPDATE":
                ...
                assert margin_data["available_margin"] >= 0, (
                    "Available margin should be non-negative"
                )
                ...
                if margin_data["total_value"] > 0:
                    ...
                    assert margin_data["margin_used"] <= max_allowed_margin, (
        ...
        try:
            ...
            while not position_update_found and attempts < max_attempts:
                try:
                    ...
                    if (update.get("type") == "PORTFOLIO_UPDATE" and 
                        update.get("subtype") == "POSITION_CHANGE"):
                        
                        # Validate position change structure
                        data = update["data"]
                        assert "position_updates" in data, (
                        ...
                        for pos_update in position_updates:
                            ...
                            assert isinstance(pos_update.get("old_size"),
                                              (int, float))
                            assert isinstance(pos_update.get("new_size"),
                                              (int, float))
                            assert pos_update.get("change_type") in [
                                "OPENED", "INCREASED", "DECREASED", "CLOSED"
                            ]
                        ...
                        if "risk_metrics" in data:
                            ...
                            assert isinstance(risk.get("var_1day"),
                                              (int, float, type(None)))
                            assert isinstance(risk.get("sharpe_ratio"),
                                              (int, float, type(None)))
    ...
    async def test_portfolio_update_frequency(self):
        ...
        try:
            ...
            while (len(update_timestamps) < target_updates and 
    ...
    async def test_portfolio_data_consistency(self):
        ...
        try:
            ...
            for i in range(1, len(sequential_updates)):
                ...
                previous = sequential_updates[i-1]["data"][
                    "portfolio_summary"
                ]
    ...
    async def test_portfolio_subscription_cleanup(self):
        ...
        try:
            ...
            unsubscribe_response = await asyncio.wait_for(
                websocket.recv(), timeout=5.0
            )
