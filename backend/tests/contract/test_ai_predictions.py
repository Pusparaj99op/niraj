...
class TestAIPredictionsContract:
    ...
    def test_get_predictions_with_symbol_filter_contract(
        ...
        response: Response = client.get(
            f"/api/v1/ai/predictions?symbol={test_symbol}"
        )
    ...
    def test_get_predictions_authorization_contract(
        self, client: TestClient
    ) -> None:
        """
    ...
    def _validate_prediction_schema(self, prediction: dict) -> None:
        ...
        required_string_fields = [
            "prediction_id", "symbol", "strategy_id", "timestamp",
            "predicted_direction", "reasoning"
        ]
        ...
        assert "market_features" in prediction, (
            "Prediction must contain 'market_features'"
        )
        assert isinstance(prediction["market_features"], dict), (
        ...
        for field in boolean_fields:
            assert field in prediction, f"Prediction must contain '{field}'"
            assert isinstance(prediction[field], bool), (
        ...
        if "was_correct" in prediction and prediction["was_correct"] is not None:
            assert isinstance(prediction["was_correct"], bool), (
