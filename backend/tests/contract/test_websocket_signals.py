...
class TestWebSocketTradeSignalsContract:
    ...
    async def subscribe_to_trade_signals(
        ...
        if (response.get("type") != "subscription_response" or
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                signal_raw = await asyncio.wait_for(
                    websocket.recv(),
                    timeout=30.0  # Signals may be less frequent
                )
                ...
                data = signal["data"]
                required_fields = [
                    "signal_id", "strategy_id", "strategy_name",
                    "symbol", "signal_type", "confidence", "strength",
                    "entry_price", "suggested_quantity"
                ]
                
                for field in required_fields:
                    assert field in data, (
                        f"Trade signal must contain '{field}'"
                    )
                ...
                assert isinstance(entry_price, (int, float)), (
                ...
                assert isinstance(quantity, int), (
                    "suggested_quantity must be integer"
                )
                assert quantity > 0, (
                    "suggested_quantity must be positive"
                )

                # Verify optional fields
                optional_fields = [
                    "stop_loss", "take_profit", "risk_amount",
                    "expected_duration", "ai_reasoning", "market_context"
                ]
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (asyncio.get_event_loop().time() - start_time <
                       collection_time and len(signals_received) < 5):
    ...
    async def test_websocket_trade_signal_confidence_filtering(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (asyncio.get_event_loop().time() - start_time <
                       collection_time and len(high_confidence_signals) < 3):
    ...
    async def test_websocket_trade_signal_market_context(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (asyncio.get_event_loop().time() - start_time <
                       timeout_seconds and not signal_with_context):
                    ...
                    try:
                        ...
                        if (message["type"] == "trade_signal" and
                                "market_context" in message["data"]):
                ...
                if signal_with_context:
                    ...
                    expected_fields = [
                        "trend", "volatility", "volume_profile",
                        "sector_sentiment"
                    ]
                    ...
                    if "volatility" in context:
                        ...
                        assert volatility in valid_vol, (
    ...
    async def test_websocket_trade_signal_ai_reasoning(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (asyncio.get_event_loop().time() - start_time <
                       timeout_seconds and not signal_with_reasoning):
                ...
                if signal_with_reasoning:
                    ...
                    assert len(reasoning) >= 10, (
                        "AI reasoning should be descriptive"
                    )

                    # Should contain trading-related keywords
                    trading_keywords = [
                        "bullish", "bearish", "support", "resistance",
                        "momentum", "volume", "trend", "analysis",
                        "pattern", "indicator", "signal", "strength"
                    ]
    ...
    async def test_websocket_trade_signal_risk_parameters(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                while (asyncio.get_event_loop().time() - start_time <
                       timeout_seconds and not signal_with_risk):
                    ...
                    try:
                        ...
                        if (message["type"] == "trade_signal" and
                                all(field in message["data"] for field in
    ...
    async def test_websocket_trade_signal_subscription_lifecycle(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                # Phase 1: Subscribe
                subscription_id = await self.subscribe_to_trade_signals(
                    websocket
                )
                ...
                assert len(signals_after_unsub) == 0, (
    ...
    async def test_websocket_trade_signal_invalid_strategy_id(self) -> None:
        ...
        with pytest.raises((ConnectionError, ConnectionRefusedError, OSError)):
            ...
            try:
                ...
                assert has_active or has_failed, (
                    "Should have subscription response"
                )
                ...
                if has_failed:
                    ...
                    assert ("strategy" in error_msg or
