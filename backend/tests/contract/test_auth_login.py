...
class TestAuthLoginContract:
    ...
    def test_login_invalid_credentials_contract(
        ...
        assert "error" in response_json, (
            "401 response must contain 'error' field"
        )
    ...
    def test_login_missing_username_contract(self, client: TestClient) -> None:
        ...
        # Act
        response: Response = client.post(
            "/api/v1/auth/login", json=incomplete_data
        )
    ...
    def test_login_empty_request_body_contract(
        self, client: TestClient
    ) -> None:
        """
    ...
    def test_login_wrong_http_method_contract(
        self, client: TestClient
    ) -> None:
        """
